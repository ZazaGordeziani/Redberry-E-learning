...
export default function EnrolledCourseCard({
    ...
    return (
        <article
            className="mx-auto box-border flex min-h-73.75 w-full max-w-155.75 flex-col gap-4 rounded-xl border border-[#F0F0F0] bg-white p-5"
            aria-label={title}
        >
            <div className="flex min-h-0 min-w-0 flex-col gap-4 sm:flex-row">
                <div className="relative mx-auto h-47.75 w-full max-w-67.25 shrink-0 overflow-hidden rounded-[10px] bg-[#F0F0F0] sm:mx-0 sm:w-67.25">
                    {imageUrl ? (
                        <img
                            src={imageUrl}
                            alt=""
            ...
            <div className="flex shrink-0 flex-row items-center justify-between gap-4">
                ...
                {courseId != null ? (
                    <Link
                        to={`/courses/${courseId}`}
                        onClick={onNavigate}
                        className="font-inter inline-flex h-12 w-29.25 shrink-0 items-center justify-center rounded-lg border border-[#958FEF] bg-white text-center text-[16px] leading-6 font-medium text-[#4F46E5] transition-colors hover:bg-[#F5F5FF]"
                    >
                        View
                    </Link>
                ) : (
